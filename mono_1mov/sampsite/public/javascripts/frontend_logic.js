...
$("#showStoredData").click(function(event) {
    ...
    if (storedCodesCheck) {
        ...
        for (var i in storedCodes) {
            ...
            var td2 = "<td>" + storedCodes[i]["user"] + "</td>";
            var td3 = "<td>" + storedCodes[i]["redeemedOn"] + "</td></tr>";
...
$("#submit").click(function(event){
    // for(var codeObj in storedCodes){
    //     console.log(codeObj);
    //     $.ajax({
    //         type:"POST",
    //         url: "mongodb://localhost:27017/sampsite", //mongodb://localhost:27017/sampsite" //or localhost 27017
    //         data: JSON.stringify({code: codeObj.code, user:codeObj.user, redeemedOn: codeObj.redeemedOn}),
    //         contentType: "application/json"
    //     });
    //     $.post("//localhost:27017", {code: codeObj.code, user:codeObj.user, redeemedOn: codeObj.redeemedOn});
    // };
});
...
//https://api.jquery.com/unload/ <-- close browser
        // this.parent().append(
        //            "<input id='hiddenForServer' type='text' name='user', value='null'><input id='hiddenForServer' type='text' name='redeemedOn', value='null'>"
        //       )});
