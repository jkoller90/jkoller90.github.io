...
var updateSeekBarWhileSongPlays = function () {
    if (currentSoundFile) {
        // timeupdate bound to currentSoundFile; fires repeatedly during playback
        currentSoundFile.bind('timeupdate', function (event) {
            
            
            // fills bar w/ completed variable, seekBarFillRatio
            var seekBarFillRatio = this.getTime() / this.getDuration();
            var $seekBar = $('.seek-control .seek-bar');
            updateSeekPercentage($seekBar, seekBarFillRatio);
        });
        console.log(currentSoundFile.getTime(), "time from in clickhandler");
        setTimeout(setCurrentTimeInPlayer($(".current-time")), 1000);    
...
//sets text of element (.total-time) to the length of the song 
...
//		});
//    
    $(totalTime).text(buzz.toTimer(currentSoundFile.getDuration()));
}       
//translates to seconds
function filterTimeCode(timeInSeconds){
    return Math.floor(parseFloat(timeInSeconds));
}
